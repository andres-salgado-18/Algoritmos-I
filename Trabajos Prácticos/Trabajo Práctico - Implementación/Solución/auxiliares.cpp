...
gps desviarPunto(gps p, double desvioMtsLatitud, double desvioMtsLongitud){
    ...
    double new_latitude = lat - (dx / radioTierra) * (180 / M_PI);
    double new_longitude = lon + (dy / radioTierra) * (180 / M_PI) / cos(lat * M_PI / 180);
...
bool haySalto(nombre n1, nombre n2) {
    ...
    int n2_0 = get<0>(n2);
    int n2_1 = get<1>(n2);
    float distanciaNombres = sqrt(pow(abs(n1_0 - n2_0), 2) + pow(abs(n1_1 - n2_1), 2));
    return (distanciaNombres > sqrt(2));
}
...
/*
tuple<tiempo, gps> generarMedicion2(tiempo tiempoDelError , gps p, gps q, float v_med, gps desde, tiempo tiempoP, tiempo tiempoQ){
    gps pq = restarGPS(q, p);
    gps pq_versor = multiplicarPorEscalar(pq, (1 / norma(pq)));
    gps pq_versor_escalado = multiplicarPorEscalar(pq_versor, (distEnKM(p,q)/1000)*(abs(tiempoDelError-tiempoP)/abs(tiempoQ - tiempoP)));
    gps r = sumarGPS(desde, pq_versor_escalado);
    tuple<tiempo,gps> medicionCorregida = make_tuple(tiempoDelError, r);
    return medicionCorregida;
}
...
void corregirPunto(viaje& v, int k){
    ...
    if (obtenerTiempo(v[k]) < obtenerTiempo(puntoCorrecto1) && obtenerTiempo(v[k]) < obtenerTiempo(puntoCorrecto2)){
        v[k] =  generarMedicion(obtenerTiempo(v[k]), obtenerPosicion(puntoCorrecto2) , obtenerPosicion(puntoCorrecto1), v_media, obtenerPosicion(puntoCorrecto2),
                                obtenerTiempo(puntoCorrecto2), obtenerTiempo(puntoCorrecto1));
    }
    else if (obtenerTiempo(v[k]) > obtenerTiempo(puntoCorrecto1) && obtenerTiempo(v[k]) < obtenerTiempo(puntoCorrecto2)){
        v[k] = generarMedicion(obtenerTiempo(v[k]),obtenerPosicion(puntoCorrecto1) , obtenerPosicion(puntoCorrecto2), v_media, obtenerPosicion(puntoCorrecto1), obtenerTiempo(puntoCorrecto1), obtenerTiempo(puntoCorrecto2));
    }
    else if (obtenerTiempo(v[k]) > obtenerTiempo(puntoCorrecto1) && obtenerTiempo(v[k]) > obtenerTiempo(puntoCorrecto2)){
        v[k] = generarMedicion(obtenerTiempo(v[k]),obtenerPosicion(puntoCorrecto2) , obtenerPosicion(puntoCorrecto1), v_media, obtenerPosicion(puntoCorrecto1), obtenerTiempo(puntoCorrecto2), obtenerTiempo(puntoCorrecto1));
    }
    */
    if (obtenerTiempo(v[k]) < obtenerTiempo(puntoCorrecto1) && obtenerTiempo(v[k]) < obtenerTiempo(puntoCorrecto2)){
        v[k] =  generarMedicion(obtenerTiempo(v[k]), obtenerPosicion(puntoCorrecto2) , obtenerPosicion(puntoCorrecto1), v_media, obtenerPosicion(puntoCorrecto2));
...
bool sonGrillasIGuales(grilla g0, grilla g1){
    if (g0.size() != g1.size()){return false;}
    for (int i = 0; i < g0.size(); i++){
        if (cantidadApariciones(g0[i], g0) != cantidadApariciones(g1[i], g1)){
            return false;
        }
    }
    return true;
